...
# ---------- 通道内层次路径：GAT + DiffPool，输出节点级多尺度表征 ----------
class HierarchicalChannelPath(nn.Module):
    ...
    def __init__(self, hidden_dim, num_scales, num_clusters=8):
        ...
        self.micro_gnn = GATConv(hidden_dim, hidden_dim)
        self.pool_gnn = GATConv(hidden_dim, num_clusters)
        self.meso_gnn = GATConv(hidden_dim, hidden_dim)
