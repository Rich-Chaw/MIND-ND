...
def _get_init_features(g: Batch, num_features, positional_encoding, handcrafted_features=False):
    ...
    else: x_init = torch.ones(g.total_nodes, num_features, device=device, dtype=torch.float32)
    if positional_encoding == 'RW':
        from utils.graph_data import random_walk_positional_encoding
        PE = random_walk_positional_encoding(g, num_features, device)
        x_init = torch.cat([x_init, PE], dim=1)
    return x_init
